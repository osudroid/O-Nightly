...
public class AttachmentServiceApiBuilder<TDb, ELogger, UInput, TransformOutput, HandlerOutput, Output>
    ...
    public (
        ...
        return (DbCreates, LoggerCreates, ValidationHandler, TransformHandler, Handler, OutputHandler);
    }

    public async Task<Transaction<Output>> ToServiceAndRun(UInput input, CancellationToken token = default) {
        return await AttachmentServiceApi<TDb, ELogger, UInput, TransformOutput, HandlerOutput, Output>.RunAsync(this,
            input, token
        );
    }
}