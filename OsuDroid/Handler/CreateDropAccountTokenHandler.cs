...
public class CreateDropAccountTokenHandler : IHandler<NpgsqlCreates.DbWrapper,LogWrapper,ControllerPostWrapper<CreateDropAccountTokenDto>,OptionHandlerOutput<ViewCreateDropAccountTokenRes>> {
    public async ValueTask<Result<OptionHandlerOutput<ViewCreateDropAccountTokenRes>, string>> Handel(NpgsqlCreates.DbWrapper dbWrapper, LogWrapper logger, ControllerPostWrapper<CreateDropAccountTokenDto> request) {
        ...
        if (cookieTokenWithUserIdOption.IsNotSet()) {
            return Result<OptionHandlerOutput<ViewCreateDropAccountTokenRes>, string>.Err(TraceMsg.WithMessage("Cookie Error"));
        }
        ...
        if (!resultValidatePassword.Ok().PasswordIsValid)
            return Result<OptionHandlerOutput<ViewCreateDropAccountTokenRes>, string>.Ok(
                OptionHandlerOutput<ViewCreateDropAccountTokenRes>.With(ViewCreateDropAccountTokenRes.PasswordIsFalse()));
        ...
        if (res == EResult.Err) {
            return res.ConvertTo<OptionHandlerOutput<ViewCreateDropAccountTokenRes>>();
        }
        return Result<OptionHandlerOutput<ViewCreateDropAccountTokenRes>, string>.Ok(OptionHandlerOutput<ViewCreateDropAccountTokenRes>.With(ViewCreateDropAccountTokenRes.NoError()));
    }
}