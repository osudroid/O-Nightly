...
public class RemoveCookieHandler 
    ...
    public async ValueTask<Result<OptionHandlerOutput<ApiTypes.ViewWork>, string>> Handel(NpgsqlCreates.DbWrapper dbWrapper, LogWrapper logger, ControllerWrapper request) {
        var db = dbWrapper.Db;
        var controller = request.Controller;

        var cookieOption = controller.GetCookie();
        if (cookieOption.IsNotSet()) {
            return Result<OptionHandlerOutput<ApiTypes.ViewWork>, string>.Ok(OptionHandlerOutput<ApiTypes.ViewWork>.With(new ApiTypes.ViewWork() {
                HasWork = true
            }));
        }
