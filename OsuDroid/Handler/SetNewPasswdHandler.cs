...
public class SetNewPasswdHandler
    : IHandler<NpgsqlCreates.DbWrapper, LogWrapper, ControllerPostWrapper<SetNewPasswdDto>, OptionHandlerOutput<ViewWebReplacePasswordWithToken>>{
    
    public async ValueTask<Result<OptionHandlerOutput<ViewWebReplacePasswordWithToken>, string>> Handel(NpgsqlCreates.DbWrapper dbWrapper, LogWrapper logger, ControllerPostWrapper<SetNewPasswdDto> request) {
        var db = dbWrapper.Db;
        ...
        if (found.Ok().IsNotSet()) {
            return Result<OptionHandlerOutput<ViewWebReplacePasswordWithToken>, string>.Ok(OptionHandlerOutput<ViewWebReplacePasswordWithToken>.With(new ViewWebReplacePasswordWithToken() {
                Work = false,
        ...
        var hashPasswordResult = OsuDroidLib.Lib.PasswordHash.HashWithBCryptPassword(prop.NewPasswd);
        ...
        return Result<OptionHandlerOutput<ViewWebReplacePasswordWithToken>, string>
            .Ok(OptionHandlerOutput<ViewWebReplacePasswordWithToken>.With(new () {
            Work = true,
            ErrorMsg = ""
        }));
    }
