...
public class ResetPasswdAndSendEmailHandler
    : IHandler<NpgsqlCreates.DbWrapper, LogWrapper, ControllerPostWrapper<ResetPasswdAndSendEmailDto>, OptionHandlerOutput<ViewResetPasswdAndSendEmail>>{

    public async ValueTask<Result<OptionHandlerOutput<ViewResetPasswdAndSendEmail>, string>> Handel(
        NpgsqlCreates.DbWrapper dbWrapper, LogWrapper logger, ControllerPostWrapper<ResetPasswdAndSendEmailDto> request) {
        ...
        if (sendResult == EResult.Err) {
            return sendResult.ConvertTo<OptionHandlerOutput<ViewResetPasswdAndSendEmail>>();
        }
