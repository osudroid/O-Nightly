...
public class GetAllPatreonHandler : IHandler<NpgsqlCreates.DbWrapper, LogWrapper, ControllerWrapper,
    OptionHandlerOutput<List<ViewUsernameAndId>>> {
    public async ValueTask<Result<OptionHandlerOutput<List<ViewUsernameAndId>>, string>> Handel(
        ...
        if (result == EResult.Err) return result.ChangeOkType<OptionHandlerOutput<List<ViewUsernameAndId>>>();
