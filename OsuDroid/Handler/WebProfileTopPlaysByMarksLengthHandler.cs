...
public class WebProfileTopPlaysByMarksLengthHandler : IHandler<NpgsqlCreates.DbWrapper,LogWrapper,ControllerGetWrapper<UserIdBox>,OptionHandlerOutput<ViewPlaysMarksLength>> {
    public async ValueTask<Result<OptionHandlerOutput<ViewPlaysMarksLength>, string>> Handel(NpgsqlCreates.DbWrapper dbWrapper, LogWrapper logger, ControllerGetWrapper<UserIdBox> request) {
        var db = dbWrapper.Db; 
