...
public class GetAvatarHashesByUserIdsHandler
    : IHandler<NpgsqlCreates.DbWrapper, LogWrapper, ControllerPostWrapper<AvatarHashesByUserIdsDto>,
        OptionHandlerOutput<ViewAvatarHashes>> {
    public async ValueTask<Result<OptionHandlerOutput<ViewAvatarHashes>, string>> Handel(
        NpgsqlCreates.DbWrapper dbWrapper,
        LogWrapper logger,
        ControllerPostWrapper<AvatarHashesByUserIdsDto> request) {
        var db = dbWrapper.Db;
