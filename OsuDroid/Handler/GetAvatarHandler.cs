...
public class GetAvatarHandler 
    : IHandler<NpgsqlCreates.DbWrapper, LogWrapper, ControllerGetWrapper<HttpGet.GetAvatar>, ImageWrapper> {
    
    public async ValueTask<Result<ImageWrapper, string>> Handel(NpgsqlCreates.DbWrapper dbWrapper, LogWrapper logger, ControllerGetWrapper<GetAvatar> request) {
        var db = dbWrapper.Db;
