...
public class WebLoginTokenHandler
    ...
    public async ValueTask<Result<OptionHandlerOutput<ViewWebLoginToken>, string>> Handel(NpgsqlCreates.DbWrapper dbWrapper, LogWrapper logger, ControllerWrapper request) {
        var db = dbWrapper.Db;
        ...
        Random random = new((int)(now.ToBinary() % Int16.MaxValue));
        ...
        if (tokenResult == EResult.Err) {
            return tokenResult.ConvertTo<OptionHandlerOutput<ViewWebLoginToken>>();
        }
