...
public class ResetPasswdAndSendEmailValidation : IValidationHandler<NpgsqlCreates.DbWrapper, LogWrapper,
    ControllerPostWrapper<PostResetPasswdAndSendEmail>> {
    public ValueTask<Result<bool, string>> Validate(
        NpgsqlCreates.DbWrapper db,
        LogWrapper logger,
        ControllerPostWrapper<PostResetPasswdAndSendEmail> input) {
