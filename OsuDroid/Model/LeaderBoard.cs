...
public static class LeaderBoard {
    public static async Task<Result<List<Entities.LeaderBoardUser>, string>> AnyRegionAsync(NpgsqlConnection db,
        int limit) {
        return (await QueryUserStats.LeaderBoardNoFilter(db, limit > 100 ? 100 : limit < 1 ? 50 : limit))
            .Map(x => x.ToList());
    }

    public static async Task<Result<List<Entities.LeaderBoardUser>, string>> FilterRegionAsync(
    ...
    public static async Task<Result<Option<Entities.LeaderBoardUser>, string>> UserAsync(NpgsqlConnection db,
        long userId) {
        return await QueryUserStats.LeaderBoardUserRank(db, userId);
    }

    public static async Task<Result<List<Entities.LeaderBoardUser>, string>> SearchUserWithRegionAsync(
        NpgsqlConnection db,
    ...
    public static async Task<Result<List<Entities.LeaderBoardUser>, string>> SearchUserAsync(
