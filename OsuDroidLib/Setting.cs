...
public static class Setting {
    ...
    private static async Task<ResultErr<string>> LoadFromDbAsync() {
        ...
        foreach (var res in settingsResult.Where(res => res == EResult.Err)) return res;

        var settings = settingsResult.Select(x => x.Ok()).ToList();

        Domain_Name = settings.First(s => s is
            { SettingMainKey: ESettingMainKey.Domain, SettingSubKey: ESettingSubKey.Name }
        );
        APK_SignKey = settings.First(s => s is
            { SettingMainKey: ESettingMainKey.APK, SettingSubKey: ESettingSubKey.SignKey }
        );
        GeoIp_LicenseKey = settings.First(s => s is
            { SettingMainKey: ESettingMainKey.GeoIp, SettingSubKey: ESettingSubKey.LicenseKey }
        );
        GeoIp_UserId = settings
                       .First(s => s is { SettingMainKey: ESettingMainKey.GeoIp, SettingSubKey: ESettingSubKey.UserId })
                       .ValueToInt();
        RequestHash_Keyword = settings.First(s => s is
            { SettingMainKey: ESettingMainKey.RequestHash, SettingSubKey: ESettingSubKey.Keyword }
        );
        Password_Seed = settings.First(s => s is
            { SettingMainKey: ESettingMainKey.Password, SettingSubKey: ESettingSubKey.Seed }
        );
        Password_MinLength = settings.First(s => s is {
            SettingMainKey: ESettingMainKey.Password,
            SettingSubKey: ESettingSubKey.MinLength
        }
    )
    .ValueToInt();
Password_BCryptSalt = settings.First(s => s is {
SettingMainKey: ESettingMainKey.Password,
SettingSubKey: ESettingSubKey.BCryptSalt
}
)
.ValueToInt();
Email_NoReplay = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Email, SettingSubKey: ESettingSubKey.NoReplay }
);
Email_NoReplayPassword = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Email, SettingSubKey: ESettingSubKey.NoReplayPassword }
);
Email_NoReplaySmtpAddress = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Email, SettingSubKey: ESettingSubKey.NoReplaySmtpAddress }
);
Email_NoReplayUsername = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Email, SettingSubKey: ESettingSubKey.NoReplayUsername }
);
Patreon_ClientId = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Patreon, SettingSubKey: ESettingSubKey.ClientId }
);
Patreon_ClientSecret = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Patreon, SettingSubKey: ESettingSubKey.ClientSecret }
);
Patreon_AccessToken = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Patreon, SettingSubKey: ESettingSubKey.AccessToken }
);
Patreon_RefreshToken = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Patreon, SettingSubKey: ESettingSubKey.RefreshToken }
);
Patreon_CampaignId = settings.First(s => s is {
SettingMainKey: ESettingMainKey.Patreon,
SettingSubKey: ESettingSubKey.CampaignId
}
)
.ValueToInt();
Log_DbName = settings.First(s => s is
{ SettingMainKey: ESettingMainKey.Log, SettingSubKey: ESettingSubKey.DbName }
);
Log_SaveInDb = settings
.First(s => s is { SettingMainKey: ESettingMainKey.Log, SettingSubKey: ESettingSubKey.SaveInDb })
.ValueToBool();
Log_Ok = settings.First(s => s is { SettingMainKey: ESettingMainKey.Log, SettingSubKey: ESettingSubKey.Ok })
.ValueToBool();
Log_Debug = settings
.First(s => s is { SettingMainKey: ESettingMainKey.Log, SettingSubKey: ESettingSubKey.Debug })
.ValueToBool();
Log_Error = settings
.First(s => s is { SettingMainKey: ESettingMainKey.Log, SettingSubKey: ESettingSubKey.Error })
.ValueToBool();
Log_RequestJsonPrint = settings.First(s => s is {
SettingMainKey: ESettingMainKey.Log,
SettingSubKey: ESettingSubKey.RequestJsonPrint
}
)
.ValueToBool();
UserAvatar_SizeLow = settings.First(s => s is {
SettingMainKey: ESettingMainKey.UserAvatar,
SettingSubKey: ESettingSubKey.SizeLow
}
)
.ValueToInt();
UserAvatar_SizeHigh = settings.First(s => s is {
SettingMainKey: ESettingMainKey.UserAvatar,
SettingSubKey: ESettingSubKey.SizeHigh
}
)
.ValueToInt();
LoginToken_ValidTimeInMin = settings.First(s => s is {
SettingMainKey: ESettingMainKey.LoginToken,
SettingSubKey: ESettingSubKey.ValidTimeInMin
}
)
.ValueToInt();
LoginToken_TokenSize = settings.First(s => s is {
SettingMainKey: ESettingMainKey.LoginToken,
SettingSubKey: ESettingSubKey.TokenSize
}
)
.ValueToInt();
...
try {
OsuDroidSecurityDll = Config["OSUDROID_SECURITY_DLL"] ?? "";
}
catch (Exception) {
OsuDroidSecurityDll = "";
}
