...
public static class QueryPatron {
    public static async Task<ResultErr<string>> SyncPatronMembersByEmails(
        NpgsqlConnection db,
        IReadOnlyList<string> activeSupportEmails) {
        var sql0 = @"
UPDATE Patron
...
";
        ...
        return await ((ResultErr<string>)
            await db.SafeQueryAsync(sql0,
                new { PatronEmail = activeSupportEmails.ToArray() }
            )).AndAsync(async () => {
                return await db.SafeQueryAsync(sql1, activeSupportEmails.Select(x =>
                        new Patron { PatronEmail = x, ActiveSupporter = true }
                    )
                );
            }
        );
    ...
    public static async Task<ResultErr<string>> SyncPatronMember(NpgsqlConnection db, Member mem) {
        var sql = @"
INSERT INTO Patron (PatronEmail, ActiveSupporter) 
VALUES (@PatronEmail, @ActiveSupporter)
ON CONFLICT DO UPDATE SET ActiveSupporter = @ActiveSupporter";
    ...
    public static async Task<Result<IEnumerable<(string Username, long UserId)>, string>> GetPatronUser(
        NpgsqlConnection db) {
        var sql = @"
...
";
    ...
    public static async Task<Result<Option<Patron>, string>> GetByPatronEmailAsync(
        NpgsqlConnection db,
        string patronEmail) {
        var sql = @"
SELECT * 
