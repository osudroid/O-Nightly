...
public static class QueryUserGroupPrivilege {
    public static async Task<Result<UserGroupPrivilegeDto, string>> GetGroupPrivileges(NpgsqlConnection db, long userId) {
        try {
            var s = db.Query(@$"
SELECT p.*, gpp.*, gp.* 
FROM privilege p
...
").ToArray();
            ...
            foreach (var rowD in s) {
                ...
                if (userDto.GroupPrivileges.TryGetValue(groupPrivilege.GroupPrivilegeId, out groupPrivilegesDto) == false) {
